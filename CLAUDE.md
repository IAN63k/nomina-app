...
- **Output**: Word (`.docx` single, or a ZIP via `jszip`) or **PDF**. PDF is generated **100% in the browser** (`src/services/cartasPdfClient.ts`): the merged `.docx` is rendered with `docx-preview` into a hidden container and each page is captured to PDF via `html2canvas-pro` (supports Tailwind v4 `oklch`) + `jsPDF`. Single → one `.pdf`; multiple → ZIP. The libs are dynamically imported so they don't bloat the initial bundle. This is client-only **on purpose** — prod runs on Vercel/serverless where LibreOffice isn't available; a previous server route (`soffice`) was removed. PDFs are image-based (faithful to the template, ~250 KB/page, text not selectable). `generarPdf` takes an `onProgress(done, total)` callback for the UI.
