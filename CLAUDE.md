...
### Horas Extra — tope semanal (44h/37h hasta jul-2026, luego 42h/35h por Ley 2101)

Colombian rule: the ordinary weekly journey is **44h** normal, **until `LEY_2101_CUTOVER` (2026-07-14)**; from that date it drops to **42h** (Ley 2101), for médicos and auxiliares alike. On top of that base, **each distinct holiday the week contains subtracts 7h** (`HOURS_PER_FESTIVO`) from the cap — one holiday → 37h/35h, two holidays in the same week → 30h/28h, etc. Anything worked beyond the applicable cap that week is reclassified as overtime (hora extra), **exclusive** of the ordinary night recargo (a given hour is either ordinary-with-recargo or extra, never both).

- Engine: `recargoEngine.ts` builds per-date segments, then a weekly pass (`buildTurnoRows`) groups by `(medico, Mon–Sun week)`. It accumulates worked hours chronologically and, once the weekly cap (`getWeeklyCap(monday, festivoCount)`) is reached, splits the crossing shift into ordinary + extra and classifies the excess. **Caveat**: hours logged without a shift code (no M/T/N → no schedule) can't be classified into a franja, so they pass through as concepto 0 and do **not** count toward the cap; fix in the programación if they should generate overtime.
- **Ley 2101 cutover**: the base cap (44 vs. 42) is chosen per week by whether **any day of that week** (Mon–Sun, i.e. effectively its Sunday, the latest day) falls on/after `LEY_2101_CUTOVER = "2026-07-14"` — a week straddling the cutover already uses the new base, even though its Monday is earlier. `WEEKLY_ORDINARY_CAP` (44) is the pre-cutover base; `WEEKLY_ORDINARY_CAP_LEY2101` (42) is post-cutover. `WEEKLY_FESTIVO_CAP`/`WEEKLY_FESTIVO_CAP_LEY2101` (37/35) are just the one-holiday case of the `HOURS_PER_FESTIVO` formula, kept as named constants for reference.
- Holidays come from `src/services/festivosColombia.ts` (`isFestivoColombia`: fixed + Ley Emiliani + Easter-based), applied to **both** módulos. Auxiliares' `/DF` reinforces it. Plain Sundays do **not** count as a holiday for this reduction (they're already outside the ordinary journey by definition). Médicos now also honors calendar holidays for recargo/extra classification.
