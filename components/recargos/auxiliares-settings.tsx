...
export function AuxiliaresSettings() {
  const { turnos, turnosCodes, setTurno, addTurno, removeTurno, resetTurnos, isDefaultTurno } = useAuxiliaresTurnos()
  ...
  const handleAddTurno = () => {
    ...
    if (!/^[A-Z0-9]{1,5}$/.test(code)) {
      setAddError("Máximo 5 caracteres alfanuméricos (ej: M3, T4, ND)")
      return
    }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-semibold tracking-tight">Turnos Auxiliares</h3>
      ...
      {/* Column headers */}
      <div className="grid grid-cols-[52px_1fr_1fr_56px_24px] items-center gap-1.5 px-0.5">
      ...
      <div className="flex flex-col gap-2">
        {sortedCodes.map((code) => {
          ...
          return (
            <div
              key={code}
              className="group relative grid grid-cols-[52px_1fr_1fr_56px_24px] items-center gap-1.5 rounded-lg border border-transparent px-0.5 py-1 transition-colors hover:border-border hover:bg-muted/40"
            >
              {/* DIG badge */}
      ...
      <div className="space-y-2">
        <Label htmlFor="new-aux-turno" className="text-xs font-semibold">
      ...
      <div className="space-y-2 text-[11px] text-muted-foreground/70">
        ...
        <p>
          Los códigos con sufijo &quot;/DF&quot; en el Excel se calculan como festivo (recargo dominical).
        </p>
      </div>
    </div>
  )
